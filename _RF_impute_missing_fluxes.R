RF_impute_missing_fluxes <- 
  function(Flux, Year, predictors = NULL, use_other_fluxes_as_predictors, impute, data, N_cores, N_trees, train_mtry, mtry, K_cv = NULL, N_cv = NULL, suffix = "") {
    ...
    if (use_other_fluxes_as_predictors) {
      ...
      predictors <- predictors[-which(predictors == "Timestamp" | predictors == Flux)]
    } else {
    ...
    if (train_mtry) {
      ...
      write.csv(mtry_results, paste0("./RF_models/mtry_", Flux, "_", Year, "_", impute, "_", N_trees, "_", use_other_fluxes_as_predictors, suffix, ".csv"), row.names = FALSE)
    }
    
    saveRDS(mod_RF, paste0("./RF_models/mod_RF_", Flux, "_", Year, "_", impute, "_", N_trees, "_", use_other_fluxes_as_predictors, suffix, ".rds"))
    ...
    # write result
    write.csv(result,paste0("./RF_results/pred_RF_", Flux, "_", Year, "_", impute, "_", N_trees, "_", use_other_fluxes_as_predictors, suffix, ".csv"), row.names = F)
    ...
    ggsave(paste0("./RF_plots/RF_", Flux, "_", Year, "_", impute, "_", N_trees, "_", use_other_fluxes_as_predictors, suffix, ".png"), plot = p, width = 16, height = 9, dpi = 300)
  }